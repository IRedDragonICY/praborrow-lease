...
use crate::network::{InMemoryNetwork, RaftMessage, PeerInfo, RaftNetwork};
use crate::raft::{InMemoryStorage, RaftStorage, LogEntry, NodeId, Term};
use crate::engine::{RaftEngine, RaftConfig, ConsensusEngine};
use crate::state_machine::{KeyValueStateMachine, KvCommand, StateMachine, ReplicatedStateMachine};
...
pub struct TestCluster<T: Clone + Send + Sync + serde::Serialize + serde::de::DeserializeOwned + 'static> {
...
impl<T: Clone + Send + Sync + serde::Serialize + serde::de::DeserializeOwned + 'static> TestCluster<T> {
...
pub fn create_test_cluster() -> (
    Vec<(NodeId, InMemoryStorage<String>, mpsc::Sender<RaftMessage<String>>, mpsc::Receiver<RaftMessage<String>>)>,
) {
...
mod tests {
    ...
    async fn test_log_replication_consistency() {
        ...
        let entries = vec![
            LogEntry { term: 1, index: 1, command: "cmd1".to_string() },
            LogEntry { term: 1, index: 2, command: "cmd2".to_string() },
            LogEntry { term: 1, index: 3, command: "cmd3".to_string() },
        ];
        ...
        let consistent = if prev_log_index == 0 {
            ...
            follower_storage.get_log_entry(prev_log_index)
                .unwrap()
    ...
    async fn test_log_conflict_resolution() {
        ...
        let old_entries = vec![
            LogEntry { term: 1, index: 1, command: "old1".to_string() },
            LogEntry { term: 1, index: 2, command: "old2".to_string() },
            LogEntry { term: 2, index: 3, command: "conflict".to_string() },
        ];
        storage.append_entries(&old_entries).unwrap();
        
        // Leader sends entries with different term at index 3
        let new_entries = vec![
            LogEntry { term: 3, index: 3, command: "new3".to_string() },
            LogEntry { term: 3, index: 4, command: "new4".to_string() },
        ];
        ...
        assert_eq!(
            storage.get_log_entry(3).unwrap().unwrap().command,
            "new3"
        );
    }
    ...
    async fn test_state_machine_apply() {
        ...
        let entries = vec![
            LogEntry {
                term: 1,
                index: 1,
                command: KvCommand::Set { key: "a".to_string(), value: b"1".to_vec() },
            },
            LogEntry {
                term: 1,
                index: 2,
                command: KvCommand::Set { key: "b".to_string(), value: b"2".to_vec() },
            },
        ];
        storage.append_entries(&entries).unwrap();
