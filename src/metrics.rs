...
impl RaftMetrics {
    ...
    pub fn inc_append_entries_sent(&self) {
        self.inner.append_entries_sent.fetch_add(1, Ordering::Relaxed);
    }
    
    /// Increments the append entries received counter.
    pub fn inc_append_entries_received(&self) {
        self.inner.append_entries_received.fetch_add(1, Ordering::Relaxed);
    }
    ...
    pub fn set_election_timeout(&self, timeout_ns: u64) {
        self.inner.election_timeout_ns.store(timeout_ns, Ordering::Relaxed);
    }
    ...
    pub fn term(&self) -> u64 { self.inner.term.load(Ordering::Relaxed) }
    pub fn role(&self) -> u64 { self.inner.role.load(Ordering::Relaxed) }
    pub fn commit_index(&self) -> u64 { self.inner.commit_index.load(Ordering::Relaxed) }
    pub fn last_applied(&self) -> u64 { self.inner.last_applied.load(Ordering::Relaxed) }
    pub fn log_entries(&self) -> u64 { self.inner.log_entries.load(Ordering::Relaxed) }
    pub fn peer_count(&self) -> u64 { self.inner.peer_count.load(Ordering::Relaxed) }
    pub fn elections_total(&self) -> u64 { self.inner.elections_total.load(Ordering::Relaxed) }
    pub fn elections_won(&self) -> u64 { self.inner.elections_won.load(Ordering::Relaxed) }
    pub fn elections_lost(&self) -> u64 { self.inner.elections_lost.load(Ordering::Relaxed) }
    pub fn proposals_total(&self) -> u64 { self.inner.proposals_total.load(Ordering::Relaxed) }
    pub fn proposals_failed(&self) -> u64 { self.inner.proposals_failed.load(Ordering::Relaxed) }
    pub fn heartbeats_sent(&self) -> u64 { self.inner.heartbeats_sent.load(Ordering::Relaxed) }
    pub fn append_entries_sent(&self) -> u64 { self.inner.append_entries_sent.load(Ordering::Relaxed) }
    pub fn append_entries_received(&self) -> u64 { self.inner.append_entries_received.load(Ordering::Relaxed) }
    ...
    pub fn to_prometheus_text(&self) -> String {
        ...
        gauge!("raft_role", "Current role (0=follower, 1=candidate, 2=leader)", self.role());
        gauge!("raft_commit_index", "Current commit index", self.commit_index());
        gauge!("raft_last_applied", "Last applied log index", self.last_applied());
        gauge!("raft_log_entries", "Number of log entries", self.log_entries());
        gauge!("raft_peer_count", "Number of peers", self.peer_count());
        gauge!("raft_replication_lag", "Replication lag (commit - applied)", self.replication_lag());
        
        // Counters
        counter!("raft_elections_total", "Total number of elections", self.elections_total());
        counter!("raft_elections_won", "Number of elections won", self.elections_won());
        counter!("raft_elections_lost", "Number of elections lost", self.elections_lost());
        counter!("raft_proposals_total", "Total proposals received", self.proposals_total());
        counter!("raft_proposals_failed", "Failed proposals", self.proposals_failed());
        counter!("raft_heartbeats_sent", "Heartbeats sent (leader only)", self.heartbeats_sent());
        counter!("raft_append_entries_sent", "AppendEntries RPCs sent", self.append_entries_sent());
        counter!("raft_append_entries_received", "AppendEntries RPCs received", self.append_entries_received());
...
mod tests {
    ...
    fn test_role_metric_conversion() {
        ...
        assert_eq!(RaftRoleMetric::from(RaftRole::Follower), RaftRoleMetric::Follower);
        assert_eq!(RaftRoleMetric::from(RaftRole::Candidate), RaftRoleMetric::Candidate);
        assert_eq!(RaftRoleMetric::from(RaftRole::Leader), RaftRoleMetric::Leader);
    }
}
